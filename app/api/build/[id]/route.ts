...
export async function POST(
  ...
  const { data: cli } = await supabase
    .from("clis")
    .select(
      "id, name, provider_id, spec_content, spec_filename, config_yml, module_path, skill_notes, repo_owner, repo_name, last_commit_sha, builds_since_release, telemetry_token, telemetry_enabled, feedback_enabled"
    )
    .eq("id", id)
    .single()
